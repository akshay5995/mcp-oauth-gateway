...
from src.config.config import GatewayConfig, ConfigManager, OAuthProviderConfig, McpServiceConfig
...
class TestConfigurationConstraints:
    ...
    def test_config_multiple_providers_validation(self, tmp_path, multi_provider_config):
...
"""
        ...
        with pytest.raises(ValueError, match="Service 'calculator' specifies OAuth provider 'google'"):
            config_manager.load_config()
...
"""
        ...
        with pytest.raises(ValueError, match="Services \\['calculator'\\] require authentication"):
            config_manager.load_config()
...
class TestBackwardCompatibilityMessages:
    ...
    def test_provider_manager_helpful_error_messages(self):
        ...
        for i, config in enumerate(multi_provider_scenarios):
            with pytest.raises(ValueError) as exc_info:
    ...
    def test_service_provider_mismatch_helpful_error(self, single_google_provider_config):
